...
The `rq-dashboard` command runs the dashboard as a standalone app. Generate a starter config file in the current directory with:

```bash
rq-dashboard init
```

This writes `./rq_dashboard_config.py`. The settings you should review before running it:

- **`RQ_QUEUES`** (required)
- **`SECRET_KEY`** — auto-generated by `init`. Treat it like a password; don't commit `rq_dashboard_config.py` to version control.

If you access the dashboard through another hostname, also set:

- **`ALLOWED_HOSTS`** — defaults to `["127.0.0.1", "localhost"]`
- **`DEBUG`** — defaults to `True`. Set to `False` when exposing the dashboard beyond local development.
...
On first run you'll be prompted to create a superuser; the dashboard is then available at `http://127.0.0.1:8000/`.

Use `--host` and `--port` to bind elsewhere e.g `rq-dashboard run --host 127.0.0.1 --port 8080`.

If the config file is somewhere else, pass a relative or absolute Python file path:

```bash
rq-dashboard run --config path/to/config_file.py
rq-dashboard run -c path/to/config_file.py
```

The standalone dashboard stores its SQLite database next to the config file as `rq_dashboard.sqlite3`.

To manage dashboard users, the same `--config` option applies:

```bash
rq-dashboard createsuperuser --config path/to/config_file.py
rq-dashboard changepassword <username> --config path/to/config_file.py
```
