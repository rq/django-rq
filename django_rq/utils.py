...
def get_scheduler_statistics():
    ...
    for index, config in enumerate(QUEUES_LIST):
        ...
        if 'connection_pool' in connection and hasattr(connection['connection_pool'], 'sentinel_manager'):
            ...
            sentinels = ','.join([
                f"{s.connection_pool.connection_kwargs.get('host')}:{s.connection_pool.connection_kwargs.get('port')}"
                for s in pool.sentinel_manager.sentinels
            ])
            conn_key = f"Sentinel({sentinels})://{pool.service_name}/{pool.sentinel_manager.connection_kwargs.get('db', 0)}"
        else:
